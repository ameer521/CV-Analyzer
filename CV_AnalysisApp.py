...
def _about():
    tkinter.messagebox.showinfo('CV ANALYZER','Cv_Analyser 1.0   "An ML and NLP implementation"\n\n@ cv.analyzer@gmail.com\n\nVerifies CVs and arrange them according to users requirements.')
...
def check():
    if(pcount>=(len(li)/2)):
        ...
        tkinter.messagebox.showinfo('CV Analyzer','Analysis Completed : '
                                    'Not Good Enough')
        with open("converted_text.txt") as file:  # file "converted.txt" opened.
            stringdata = file.read()
        numbers = extract_phone_numbers(stringdata)
...
analysebtnss = ttk.Button(f1, text="Check Another",command=lambda:[check_another,raise_frame(f3)]).place(x=320, y=340)
...
ttk.Button(f4, text='Start processing', command=lambda:raise_frame(f3)).pack(pady=(210),padx=(260))
